...
def kp():
    ...
    kp_index = sw_data["Kp0"].loc[:current_datetime].iloc[-1].astype(float)
...
def plot_kp(time_frame):
    ...
    if time_frame == "Past Week":
        ago= current_datetime - timedelta(weeks=1)
    elif time_frame == "Past Month":
        ago = current_datetime - timedelta(days=30)
    elif time_frame == "Past Year":
        ago = current_datetime - timedelta(days=365)
    ...
    # Axes and formatting
    plt.title(f"Kp Index — {time_frame}", fontsize=14)
    plt.ylabel("Kp Value")
    plt.xlabel("Date (UTC)")
    plt.ylim(0, 9)
    ...
    #     ago = one_month_ago
    # else:
    #     st.error("Invalid time frame selected for Kp trend.")
